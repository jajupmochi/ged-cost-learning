...
if __name__ == "__main__":
	...
	from ged_cost_learning.utils.distances import euclid_d, man_d, classif_d
	...
	if len(sys.argv) > 1:
		...
		task_grid = ParameterGrid(
			{
				'edit_cost': Edit_Cost_List[1:2],
				'dataset': Dataset_list[0:2],
				'distance': Dis_List[0:1]
			}
		)

		unlabeled = False  # @todo: Not actually used. Automatically set in run_xp().
		mode = 'reg'
		# Run.
		for task in list(task_grid):
			...
			# output_result = 'outputs/results_GH2020.' + '.'.join(
			# 	[task['dataset'], task['edit_cost'], task['distance']]
			# ) + '.pkl'
